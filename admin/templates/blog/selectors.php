...
function showCategory( obj ) {
	var cat = $( obj ).val();
	if ( cat !== "" )
		window.location.href = '<?php echo basename($_SERVER['PHP_SELF']) ?>?category=' + urlencode(str_replace(' ', '_', cat));
	else
...
function showPost( obj, objcat ) {
	var post = $( obj ).val(),
		cat = $( objcat ).val();
	if ( post !== "" && cat !== "" )
		window.location.href = '<?php echo basename($_SERVER['PHP_SELF']) ?>?category=' + urlencode(str_replace(' ', '_', cat)) + '&post=' + post;
	else
		window.location.href = '<?php echo basename($_SERVER['PHP_SELF']) ?>';	
}
</script>
